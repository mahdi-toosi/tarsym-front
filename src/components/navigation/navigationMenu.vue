<template>
    <div
        ...
        :class="[
            !isCollapsed ? 'vsm_expanded' : 'vsm_collapsed',
            theme ? `vsm_${theme}` : '',
            rtl ? 'vsm_rtl' : ''
        ]"
        :style="[
    ...
    >
        <slot name="header" />
        <div
            @mouseover="expandthis"
            class="vsm--list"
            :style="
                scrollable && isCollapsed
                    ? [
                          { width: 'calc(100% + 17px)' },
                          rtl ? { 'margin-left': '-17px' } : {}
...
<script>
import SidebarMenuItem from "./navigationMenuItem.vue";
        ...
        mobileItemStyle() {
            return {
                ...
                dropdown: [
                    ...
                    {
                        "max-height": `calc(${this.parentHeight} - ${this
                            .mobileItemPos + this.mobileItemHeight}px - ${
    ...
    methods: {
        expandthis() {
            if (this.isCollapsed) {
                ...
                this.collapseWithHover = true;
            }
        },
        onMouseLeave() {
            // console.log(this.collapseWithHover);

            if (this.collapseWithHover) {
                this.isCollapsed = true;
                this.collapseWithHover = false;
            }
        ...
        setMobileItem({ item, itemEl }) {
            ...
            let pos =
                itemEl.getBoundingClientRect().top - sidebarTop + paddingTop;
        ...
        initParentOffsets() {
            ...
            if (this.relative) {
                ...
                this.rtl
                    ? (this.parentOffsetLeft = `${parent.offsetWidth -
                          sidebarRight +
                          parentLeft}px`)
                ...
                this.rtl
                    ? (this.parentOffsetLeft = `calc(${this.parentWidth} - ${sidebarRight}px)`)
